...
VALUES
    ...
    ('Куртка для сноуборда DC Mutiny Charocal', 'Куртка для сноуборда', 4, 7500, 'img/lot-5.jpg', NOW(), NOW() + INTERVAL 10 DAY, '100', 3),
    ('Маска Oakley Canopy', 'Маска', 6, 5400, 'img/lot-6.jpg', NOW(), NOW() + INTERVAL 10 DAY, 4);
...
SELECT l.name, start_price, img_url, MAX(sum_rate) as max_rate, c.name as category_name
   FROM lots l
INNER JOIN categories c ON l.cat_id = c.id
INNER JOIN rates r ON l.cat_id = r.lot_id
   WHERE dt_end > NOW()
